...
from PyQt6.QtWidgets import (
    ...
    QGroupBox, QSpinBox, QDoubleSpinBox, QProgressBar, QMessageBox,
    QFileDialog, QCheckBox, QComboBox, QSplitter, QFrame
)
...
class OptimizationWorker(QThread):
    ...
    def __init__(self, parameters: Dict[str, Any], generations: int = 100, population: int = 30):
        super().__init__()
    ...
    def run(self):
        ...
        try:
            ...
            self.progress.emit(f"🎯 Target: {goals.min_positions}-{goals.max_positions} positions")
            self.progress.emit(f"🎯 Target diameter: {goals.target_outer_diameter:.1f}mm")
            self.progress.emit(f"📐 Fixed GUI parameters: {gui_params.outer_diameter_mm:.1f}mm ⌀, {gui_params.inner_diameter_mm:.1f}mm inner, {gui_params.arc_angle_deg:.1f}° arc")
            self.progress.emit(f"🎯 Optimizing only track layout (width: {gui_params.track_width_mm:.1f}mm, spacing: {gui_params.track_spacing_mm:.1f}mm, gap: {gui_params.gap_width_deg:.1f}°)")
            
            # Create and run optimizer with fixed GUI parameters
            optimizer = EncoderOptimizer(goals, fixed_params=gui_params)
            
            self.progress.emit(f"🧬 Running {self.generations} generations with population {self.population}...")
            
            # Run optimization
            best_genome = optimizer.optimize(generations=self.generations, population_size=self.population)
            ...
            self.progress.emit(f"✅ Optimization complete! Best fitness: {best_genome.fitness:.3f}")
...
class EncoderControllerGUI(QMainWindow):
    ...
    def create_left_panel(self) -> QWidget:
        ...
        # Title
        title = QLabel("🎛️ Encoder Parameters")
    ...
    def create_track_group(self) -> QGroupBox:
        ...
        group = QGroupBox("🛤️ Track Layout")
        layout = QGridLayout(group)
    ...
    def create_control_buttons(self) -> QWidget:
        ...
        # Generation section
        gen_group = QGroupBox("🏗️ Generation")
        gen_layout = QVBoxLayout(gen_group)
        
        self.generate_btn = QPushButton("🏗️ Generate SCAD File")
    ...
    def create_right_panel(self) -> QWidget:
        ...
        self.log_message("Set your physical parameters and click 'Validate Parameters' to start.")
    ...
    def on_validation_finished(self, is_valid: bool, errors: list, warnings: list):
        ...
        if is_valid:
            ...
            self.validation_output.append("All parameters meet manufacturing and functional constraints.\n")
        else:
        ...
        if warnings:
            self.validation_output.append("⚠️  WARNINGS:")
        ...
        self.validation_output.append(f"  • Outer diameter: {params['outer_diameter_mm']:.1f}mm")
        self.validation_output.append(f"  • Arc angle: {params['arc_angle_deg']:.1f}°")
        self.validation_output.append(f"  • Positions: {params['num_positions']}")
        self.validation_output.append(f"  • Tracks: {params['num_tracks']}")
        self.validation_output.append(f"  • Angular resolution: {params['arc_angle_deg']/params['num_positions']:.2f}°")
    ...
    def on_optimization_finished(self, results: Dict[str, Any]):
        ...
        self.optimization_output.append(f"  • Outer diameter: {params['outer_diameter_mm']:.1f}mm")
        self.optimization_output.append(f"  • Arc angle: {params['arc_angle_deg']:.1f}°")
        self.optimization_output.append(f"  • Positions: {params['num_positions']}")
        self.optimization_output.append(f"  • Track width: {params['track_width_mm']:.1f}mm")
        self.optimization_output.append(f"  • Gap width: {params['gap_width_deg']:.1f}°")
    ...
    def generate_encoder(self):
        ...
        self.log_message("🏗️ Generating encoder SCAD file...")
        ...
        try:
            ...
            cmd = [
                ...
                "--output", str(output_dir / "gui_encoder.scad"),
                "--config", "custom",
